...
## Acclimating response of rphydro_nitrogen

No longer works with a ppfd starting from 0, not quite sure why this is...
...
ggplot() + 
  ...
  geom_line(data = df_original.m, aes(y = value, x = ppfd, color = "Numeric Answer")) +
  scale_color_manual(values = c("Line 1" = "aquamarine4", "Line 2" = "red")) +
  theme_classic() +
...
## Instantaneous response - rphydro_instantaneous_nitrogen

```{r}
acc = dat %>% filter(ppfd == 1200)

dat_inst = tibble(ppfd = seq(1,1200,length.out=50)) %>% mutate(dat = purrr::map(.x=ppfd, .f = ~rphydro_instantaneous_nitrogen(acc$vcmax25, acc$jmax25, tc, tc, .x, netrad, vpd, co2, pa, nitrogen_uptake, fapar, kphio, psi_soil, rdark, vwind, a_jmax, par_plant, par_cost, options))) %>% unnest_wider(dat)
...
ggplot() + 
  geom_line(data = df.m.inst, aes(y = value, x = ppfd, color = "Line 1")) +
  geom_line(data = df_original.m.inst, aes(y = value, x = ppfd, color = "Line 2")) +
  scale_color_manual(values = c("Line 1" = "aquamarine4", "Line 2" = "red")) +
  theme_classic() +
  theme(strip.background = element_rect(color = "white", size = 1),
        legend.title = element_blank()) +
